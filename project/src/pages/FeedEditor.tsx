...
const FeedEditor = () => {
  ...
  // Не загружаем URL автоматически, URL будет получен только при нажатии на кнопку публикации
  
  // Получаем уникальные vendor для выпадающего списка
  ...
  // Получаем уникальные значения для каждого параметра из attributes
  const attributeOptions = useMemo(() => {
    ...
    // Преобразуем Set в массивы и сортируем значения
            ...
            <h1 className="text-2xl font-semibold text-gray-900 flex items-center gap-2">
              ...
              {currentFeed.metadata?.url && (
                <a
                  ...
                  className="ml-2 text-blue-500 hover:underline flex items-center gap-1 text-base"
                  title="Открыть исходный фид"
                >
                  <ExternalLink className="h-4 w-4" />
                  <span className="hidden sm:inline truncate max-w-[200px] align-middle">{currentFeed.metadata.url}</span>
                </a>
              )}
            </h1>
